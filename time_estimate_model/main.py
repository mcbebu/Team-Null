...
def knn_regressor_a_b_lat_long_estimated_time_lookup_table():
    ...
    neigh.fit(X[:train_len], y[:train_len])

    # print('gt', y[0])
    # print('prediction', neigh.predict([[1.386478068, 103.7602565, 1.387284982, 103.7590052]]))
    print('mean squared error', mean_squared_error(y[train_len:], neigh.predict(X[train_len:])))
    print('r2 score', r2_score(y[train_len:], neigh.predict(X[train_len:])))
